...
## two real processes.
##
##   host:   "$godot" --path RetroXR res://Tools/netplay_fsa_probe.tscn -- --net-host  --fsa-gbas=4
##   client: "$godot" --path RetroXR res://Tools/netplay_fsa_probe.tscn -- --net-join=127.0.0.1 --fsa-gbas=4
##
...
func _ready() -> void:
	for arg: String in OS.get_cmdline_user_args():
		if arg == "--net-host":
			...
			gbas = clampi(int(arg.trim_prefix("--fsa-gbas=")), 1, 4)
	...
	var gc := _spawn(gc_core, gc_rom, "gc")
	_machines = [gc]
	for i in range(gbas):
	...
	print("[fsa] %s, console + %d handheld(s)" % ["HOST" if _is_host else "CLIENT", gbas])
...
func _on_peer(id: int, _info: Dictionary) -> void:
	...
	var ok: bool = NetworkManager.netplay_start_host(_machines[0], gc_core, "", owners, 3, 0)
...
func _process(_d: float) -> void:
	...
	if gc_frame > 0 and gc_frame % 600 == 0:
		var fr: Array = []
		for m: FsaMachine in _machines:
			fr.append(int(m.lib.GetFrameCount()))
		print("[fsa] frames %s desyncs=%d" % [str(fr), _desyncs.size()])
